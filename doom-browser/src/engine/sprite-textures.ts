...
function buildLatcherAngleViews(pose: EnemyPose): Texture[] {
...
function generateLatcherTexture(pose: EnemyPose): Texture {
...
function buildBossAngleViews(pose: EnemyPose): Texture[] {
  return [
...
function generateBossTexture(pose: EnemyPose, view: EnemyView, mirror: boolean): Texture {
  const canvas = document.createElement('canvas');
  canvas.width = SPRITE_TEXTURE_SIZE;
  canvas.height = SPRITE_TEXTURE_SIZE;
  const ctx = canvas.getContext('2d')!;
